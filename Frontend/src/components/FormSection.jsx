...
const FormSection = ({ langData, currentLang }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setChatReply("KrishiSakhi AI is analyzing... Please wait.");
        setIsChatOpen(true); 

        try {
            // ✅ Netlify Live Path (Full URL for safety)
            ...
            setChatReply("Error: Backend se connect nahi ho pa raha. Check Netlify Logs.");
        }
    ...
    return (
        <section className="py-12 bg-gray-50 min-h-screen">
            <div className="max-w-6xl mx-auto px-4">
                <h2 className="text-center text-4xl font-black text-green-600 mb-8 uppercase">{langData.formHeading}</h2>
                <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl flex flex-col lg:flex-row gap-10 border border-green-50">
                    <div className="flex-1 p-10 rounded-3xl bg-green-50/30 border-2 border-dashed border-green-100 flex flex-col items-center justify-center">
                        <button onClick={startVoiceRecording} className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 shadow-lg ${isRecording ? 'bg-red-500 animate-pulse' : 'bg-green-600'}`}>
                ...
                {isChatOpen && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] p-4">
                        <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden border-t-[12px] border-green-600">
                            ...
                            <div className="p-10 max-h-[50vh] overflow-y-auto whitespace-pre-wrap text-lg leading-relaxed text-gray-700">{chatReply}</div>
                        </div>
                    </div>
                )}
            </div>
